          ...
          <div className="max-w-4xl mx-auto relative z-10">
            ...
            <div className="flex flex-wrap items-center gap-4 py-6 border-y border-slate-200">
              ...
              <div className="ml-auto flex gap-3">
                <a
                  href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(`https://nexoen.de/blog/${post.slug}`)}&text=${encodeURIComponent(post.title)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="p-2 rounded-[4px] bg-slate-100 text-slate-400 hover:text-teal-600 hover:bg-teal-50 transition-colors"
                  aria-label="Auf Twitter teilen"
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z"/></svg>
                </a>
                <a
                  href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`https://nexoen.de/blog/${post.slug}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="p-2 rounded-[4px] bg-slate-100 text-slate-400 hover:text-teal-600 hover:bg-teal-50 transition-colors"
        ...
        <article className="pb-24 px-4 sm:px-6 lg:px-8 flex-1">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="mt-16 bg-gradient-to-r from-teal-600 to-teal-500 rounded-[4px] p-8 text-white text-center">
              ...
              <p className="text-teal-100 mb-6">Teste nexoen 3 Monate kostenlos – keine Kreditkarte nötig.</p>
