...
export async function getReadings(meterId: string): Promise<Reading[]> {
  ...
  const { data, error } = await supabase
    ...
    .select('*')
    .eq('meter_id', meterId)
    .order('reading_date', { ascending: false }) as { data: Reading[] | null; error: Error | null };
...
export async function updateReading(
  ...
  const { data, error } = await supabase
    ...
    .eq('id', id)
    .select()
    .single() as { data: Reading | null; error: Error | null };

  if (error || !data) {
    throw new Error(error?.message || 'Fehler beim Aktualisieren des Zählerstands');
  }
